...
const PostRow = ({ post }: { post: Post }) => {
    ...
    const remove = () => {
        if (
            !confirm(
                `Supprimer « ${post.title} » ? Cette action est définitive.`,
            )
        )
            return;
    ...
    return (
        <div className="border-border bg-card hover:border-primary/40 flex flex-col gap-4 border p-4 transition-colors sm:flex-row sm:items-center">
            <div className="bg-muted h-20 w-full shrink-0 overflow-hidden sm:w-32">
                {image ? (
                    <img
                        src={image}
                        alt=""
            ...
            <div className="min-w-0 flex-1">
                <div className="mb-1.5 flex flex-wrap items-center gap-2">
                    <span className="border-border text-muted-foreground border px-2 py-0.5 text-xs font-semibold">
                        {POST_TYPE_LABELS[post.type]}
                    </span>
                    <span
                        className={`px-2 py-0.5 text-xs font-semibold ${state.className}`}
                    >
                        {state.label}
                    </span>
                    {post.is_pinned ? (
            ...
            <div className="flex shrink-0 items-center gap-1">
                ...
                new Date(post.published_at) <= new Date() ? (
                    <a
                        ...
                        className="text-muted-foreground hover:text-primary hover:bg-accent p-2 transition-colors"
                    >
                ...
                <button
                    ...
                    className="text-muted-foreground hover:text-destructive hover:bg-accent cursor-pointer p-2 transition-colors"
                >
            ...
            <div className="pb-16">
                <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    ...
                    <Link
                        ...
                        className="bg-primary text-primary-foreground hover:bg-primary/90 inline-flex items-center gap-2 self-start px-4 py-2.5 text-sm font-semibold transition-colors"
                    >
                ...
                {posts.length === 0 ? (
                    <div className="border-border text-muted-foreground border border-dashed py-20 text-center">
